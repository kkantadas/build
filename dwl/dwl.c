	...
	if (c && !client_is_unmanaged(c)) {
		...
		if (!exclusive_focus && !seat->drag)
			client_set_border_color(c,
					(float[])COLOR(colors[SchemeSel][ColBorder]),
					(float[])COLOR(colors[SchemeSel][ColBorder]),
					(float[])COLOR(colors[SchemeSel][ColBorder]));
	}
