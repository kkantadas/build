...
static inline void
client_set_border_color(Client *c, const float color[static 4], const float scolor[static 4], const float ecolor[static 4])
{
	int i;
	for (i = 0; i < 4; i++) {
		if (border_color_type == BrdOriginal) {
			...
			wlr_scene_rect_set_color(c->borders[i], scolor);
		} else if (border_color_type == BrdEnd) {
			wlr_scene_rect_set_color(c->bordere[i], ecolor);
